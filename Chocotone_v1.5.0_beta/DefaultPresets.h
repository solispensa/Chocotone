...
void loadFactoryPresets() {
  ...
  // TAP TEMPO (special)
  {
    ...
    btn.messages[0].data2 = 127;
    btn.messages[0].rgb[0] = 0xFF;
    btn.messages[0].rgb[1] = 0xFF;
    btn.messages[0].rgb[2] = 0xFF;
  ...
  const char *noteNames[] = {"1st", "2nd", "3rd", "4th",
                             "5th", "6th", "7th", "8up"};

  for (int i = 0; i < 8; i++) {
    setProgramSelect(buttonConfigs[3][i], noteNames[i], noteValues[i], 0xFD,
                     0x00, 0x00);
  }
  ...
  // ========================================
  // (Clearing - user will configure these via Serial/Web)
